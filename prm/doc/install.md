...
agent works with Mesos worker node, pulls the contention detection model from zookeeper service periodically, and detects 
...
the csv file, it is highly recommanded that user rotates it manually or periodically with logrotate utility.
...
a single csv file, which requires user to combine all the csv files collected from those nodes into a single csv file.
